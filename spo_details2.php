...
if ($spo_header['transaction_method'] === 'Credit') {
    ...
    if ($credit_info) {
        ...
        $total_paid = (float)$total_paid_data['total_paid'];

        // Compute bad-order deductions (returns + replacements) to adjust balance
        $total_return_amount_balance = 0.0;
        $total_replacement_deduction_balance = 0.0;
        if (!empty($spo_items)) {
            foreach ($spo_items as $bo_item) {
                $bo_qty = (int)($bo_item['bad_order_qty'] ?? 0);
                if ($bo_qty > 0) {
                    $type = $bo_item['bad_order_type'] ?? 'return';
                    if ($type === 'return') {
                        $line_return_amount = isset($bo_item['return_amount']) && $bo_item['return_amount'] !== null
                            ? (float)$bo_item['return_amount']
                            : ((float)$bo_item['price'] * $bo_qty);
                        $total_return_amount_balance += $line_return_amount;
                    } elseif ($type === 'replacement') {
                        $total_replacement_deduction_balance += ((float)$bo_item['price'] * $bo_qty);
                    }
                }
            }
        }
        $total_bad_order_deductions = $total_return_amount_balance + $total_replacement_deduction_balance;

        $adjusted_total_with_interest = (float)$credit_info['total_with_interest'] - $total_bad_order_deductions;
        if ($adjusted_total_with_interest < 0) { $adjusted_total_with_interest = 0; }

        $balance = $adjusted_total_with_interest - $total_paid;
                                ...
                                <?php foreach ($spo_items as $item): ?>
                                    ...
                                    <tr class='align-middle'>
                                        ...
                                        <td class="text-center">
                                            <?php if ($item['bad_order_qty'] > 0): ?>
                                                <span class="badge bg-danger"><?php echo $item['bad_order_qty']; ?></span>
                                            <?php else: ?>
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <?php endif; ?>
                                <div class="d-flex justify-content-between mb-2">
                                    <span><strong>Bad Orders Quantity:</strong></span>
                                ...
                                <?php if ($total_bad_orders > 0): ?>
                                    <?php
                                    // Calculate bad order financial impact
                                    $total_return_amount = 0;
                                    $total_replacement_deduction = 0;
                                    $replacement_invoices = [];
                                    
                                    foreach ($spo_items as $item) {
                                        if ($item['bad_order_qty'] > 0) {
                                            if ($item['bad_order_type'] === 'return') {
                                                $total_return_amount += $item['return_amount'] ?? ($item['price'] * $item['bad_order_qty']);
                                            } elseif ($item['bad_order_type'] === 'replacement') {
                                                $total_replacement_deduction += $item['price'] * $item['bad_order_qty'];
                                                if (!empty($item['replacement_invoice'])) {
                                                    $replacement_invoices[] = $item['replacement_invoice'];
                                                }
                                            }
                                        }
                                    }
                                    ?>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span><strong>Bad Order Amount:</strong></span>
                                        <span class="text-success">₱<?php echo number_format($total_return_amount, 2); ?></span>
                                    </div>
                                    <?php if ($total_replacement_deduction > 0): ?>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span><strong>Replacement Deduction:</strong></span>
                                        <span class="text-warning">₱<?php echo number_format($total_replacement_deduction, 2); ?></span>
                                    </div>
                                    <?php endif; ?>
                                    <?php if (!empty($replacement_invoices)): ?>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span><strong>Replacement Invoices:</strong></span>
                                        <span class="text-info"><?php echo implode(', ', array_unique($replacement_invoices)); ?></span>
                                    </div>
                                    <?php endif; ?>
                                    
                                    <?php
                                    // Calculate total deductions and final amount
                                    $total_deductions = $total_return_amount + $total_replacement_deduction;
                                    if ($total_deductions > 0):
                                        if ($spo_header['transaction_method'] === 'Credit' && $credit_info) {
                                            $final_amount = $credit_info['total_with_interest'] - $total_deductions;
                                        } else {
                                            $final_amount = $total_amount - $total_deductions;
                                        }
                                    ?>
                                    <hr class="my-2">
                                    <div class="d-flex justify-content-between mb-2">
                                    ...
                                    <div class="d-flex justify-content-between mb-2">
                                        <span><strong>Final Amount Owed:</strong></span>
                                        <span class="text-primary fw-bold">₱<?php echo number_format($final_amount, 2); ?></span>
                                    </div>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                    ...
                    <div class='modal-body'>
                        <form method="POST" action="">
                            ...
                            <div class='mt-3 p-1'>
                                ...
                                <input type="number" name="transfer_qty" id="transfer_qty" class='form-control' min="1" step="1" placeholder="Enter quantity">
                                <div id="transfer_qty_error" class="text-danger small mt-1" style="display: none;"></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Positive whole number validation function for transfer quantity
    function validateTransferQuantity(input) {
        const value = input.value.trim();
        const errorDiv = document.getElementById('transfer_qty_error');
        ...
        if (value % 1 !== 0) {
            errorDiv.textContent = 'Please enter a whole number only';
            errorDiv.style.display = 'block';
            input.classList.add('is-invalid');
            return false;
        }
    ...
    // Transfer quantity validation event listeners
    const transferQtyInput = document.getElementById('transfer_qty');
    if (transferQtyInput) {
        transferQtyInput.addEventListener('input', function() {
            validateTransferQuantity(this);
        });
        
        transferQtyInput.addEventListener('blur', function() {
            validateTransferQuantity(this);
        });
    }
    
    // Form validation for transfer stock
    const transferForm = document.querySelector('form[action*="transfer-stock"]') || document.querySelector('form');
    if (transferForm) {
        transferForm.addEventListener('submit', function(e) {
            const transferQty = document.getElementById('transfer_qty');
            if (transferQty && !validateTransferQuantity(transferQty)) {
                e.preventDefault();
                return false;
            }
        });
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div class="alert alert-info">
                            ...
                            <strong>Bad Order Types:</strong>
                            <ul class="mb-0 mt-2">
                                <li><strong>Return:</strong> Return items to supplier and get refund/credit</li>
                                <li><strong>Replacement:</strong> Get replacement items from supplier</li>
                            </ul>
                        </div>
                    </div>
                    
                    <form id="badOrdersForm" method="POST" action="process_bad_orders.php">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-warning" name="process-bad-orders">
                                <i class="fas fa-save me-2"></i>Process Bad Orders
                            </button>
                        </div>
                    </form>
                </div>
